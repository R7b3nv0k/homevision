...
// Galéria elemek lekérdezése az adatbázisból (ha elérhető)
$galleryItems = [];
if (isDBConnected()) {
    try {
        $stmt = $pdo->query("SELECT * FROM media_gallery ORDER BY id ASC");
        $galleryItems = $stmt->fetchAll();
    } catch (Exception $e) {
        error_log("Galéria lekérési hiba: " . $e->getMessage());
    }
}

// Fallback elemek, ha az adatbázis üres vagy offline
if (empty($galleryItems)) {
    $galleryItems = [
        [
            'id' => 1,
            'title' => 'Villa Minimal - Budapest',
            'location' => 'Budapest',
            'image_path' => '../images/villa_minimal.jpg'
        ],
        [
            'id' => 2,
            'title' => 'Nordic Family - Győr',
            'location' => 'Győr',
            'image_path' => '../images/nordic_family.jpg'
        ]
    ];
}
?>
...
<body>
    ...
    <nav class="navbar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="media.php" class="active" data-i18n="media">Média</a></li>
    ...
    <main class="page-container">
        <h1 class="page-title" data-i18n="media_title">Már megépült otthonok (Galéria)</h1>

        <div class="media-grid">
            <?php foreach ($galleryItems as $item): ?>
            <div class="media-card">
                <img src="<?= htmlspecialchars($item['image_path']) ?>" alt="<?= htmlspecialchars($item['title']) ?>" class="plan-img" style="height: 250px;">
                <div class="plan-content">
                    <h2 class="plan-title"><?= htmlspecialchars($item['title']) ?></h2>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
